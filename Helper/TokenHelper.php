...
namespace MauticPlugin\MauticReferralsBundle\Helper;
...
class TokenHelper
{
     ...
     * @return array|string
     */
    public static function findReferrerTokens($content, $referrer, $replace = false)
    {
        if (!$referrer) {
        ...
        $tokenList    = [];
        $foundMatches = preg_match_all('/({|%7B)referrerfield=(.*?)(}|%7D)/', $content, $matches);
        ...
        if ($foundMatches) {
            foreach ($matches[2] as $key => $match) {
                ...
                $tokenList[$token] = self::getTokenValue($referrer, $alias, $defaultValue);
            }
     ...
     *
     * @param array  $tokens like ['{referrerfield=website}' => 'https://mautic.org']
     * @param string $token  like '{referrerfield=website|https://default.url}'
     *
